...
class Qwen3VLVisionAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if return_logits:
            with torch.no_grad():
                query_states = query_states[0].split(batched_num_patches.tolist(), dim=1)
                key_states = key_states[0].split(batched_num_patches.tolist(), dim=1)
                attn_logits = []
                attn_keys = []
                for query_state,key_state in zip(query_states,key_states):
                    attn_logit = torch.matmul(query_state, key_state.transpose(-1, -2))
                    attn_logit = attn_logit / (query_state.shape[-1] ** 0.5)
                    attn_logit = F.softmax(attn_logit, dim=-1)
                    # 16,256,256
                    ...
                    attn_key = key_state.view(key_state.shape[0], key_state.shape[1] // merge_size**2, merge_size**2, key_state.shape[-1]).mean(dim=2)
                    attn_key = attn_key.mean(dim=0).unsqueeze(0)

                    attn_logits.append(attn_mean)
                    attn_keys.append(attn_key)

        else:
            attn_logits = None
            attn_keys = None
        torch.cuda.empty_cache()
        return attn_output, attn_logits, attn_keys
...
class Qwen3VLVisionModel(Qwen3VLPreTrainedModel):
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor, batched_num_patches,merge_size,**kwargs) -> torch.Tensor:
        """
        Args:
        ...
        for layer_num, blk in enumerate(self.blocks):
            # Apply dyn_token before each deepstack layer
            if layer_num == len_blocks - 1:
                hidden_states, logits, attn_key = blk(hidden_states, cu_seqlens=cu_seqlens, position_embeddings=position_embeddings,batched_num_patches = batched_num_patches,merge_size=merge_size,return_logits=True,**kwargs)
            else:
                hidden_states, _, _ = blk(hidden_states, cu_seqlens=cu_seqlens, position_embeddings=position_embeddings, batched_num_patches= batched_num_patches,merge_size=merge_size,return_logits=False,**kwargs)
        ...
        return hidden_states, deepstack_feature_lists, logits, attn_key
...
class Qwen3VLModel(Qwen3VLPreTrainedModel):
    ...
    def get_video_features(
        self, pixel_values_videos: torch.FloatTensor, video_grid_thw: Optional[torch.LongTensor] = None,batched_num_patches=None
    ):
        """
        ...
        return self.get_image_features(pixel_values_videos, video_grid_thw,batched_num_patches=batched_num_patches)

    def get_image_features(self, pixel_values: torch.FloatTensor, image_grid_thw: Optional[torch.LongTensor] = None,batched_num_patches=None):
        """
        ...
        image_embeds, deepstack_image_embeds, attn_logits, attn_key = self.visual(pixel_values, grid_thw=image_grid_thw,batched_num_patches=batched_num_patches,merge_size = self.visual.spatial_merge_size)
        split_sizes = (image_grid_thw.prod(-1) // self.visual.spatial_merge_size**2).tolist()
        image_embeds = torch.split(image_embeds, split_sizes)
        return image_embeds, deepstack_image_embeds, attn_logits, attn_key
    ...
    def dyn_token_compose(
        ...
        for mm_feature,attn_mean,attn_key in zip(mm_features,attn_logits,attn_keys):
            ...
            temperature = 2
            attn_probs = torch.nn.functional.softmax(attn_mean / temperature, dim=-1)
            contextual_num = max(int(0.05 * num_tokens), 1)
            tau = float(self.tau)
            topk_indices = get_mask_entropy(attn_probs, tau=tau)
            ...
            # Only update contextual anchor tokens; token reduction via select_mask is done in _compress_visual_tokens
    ...
    def inter_slice_compse(
        self,
        pixel_values: torch.FloatTensor,
        batched_num_patches: torch.LongTensor,
        grid_sizes: torch.LongTensor,
        merge_size: torch.FloatTensor = 2,
        patch_size: torch.FloatTensor = 14,
        min_tokens: int = 1,
    ) -> torch.BoolTensor:
        ...
        _ , h, w = grid_sizes[0]
        t = len(batched_images)
        if t == 1:
            compression_masks.append(torch.ones((batched_num_patches,), dtype=torch.bool, device=pixel_values.device))

        elif t >= 1:
            # Video token compression based on pixel differences
            images = pixel_values.view(t, (h // merge_size) * (w // merge_size), -1) # (t, P, D)

            mask = []
            ref_index = 0
            for i in range(1,t):
                frame_l1 = torch.abs(images[i] - images[ref_index]).mean()
                # Keep frame if whole-frame L1 distance > gamma, otherwise discard
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if pixel_values is not None:
            select_pixel = True
            batched_num_patches = image_grid_thw.prod(dim=1).div(self.visual.spatial_merge_size ** 2).long()
            image_embeds, deepstack_image_embeds, attn_logits, attn_key = self.get_image_features(pixel_values, image_grid_thw,image_grid_thw.prod(dim=1))
            ...
            if self.slice_compose:
                compression_mask = self.inter_slice_compse(pixel_values, batched_num_patches, image_grid_thw)
            if self.token_compose:
                image_embeds,dyn_token_mask = self.dyn_token_compose(image_embeds,batched_num_patches,attn_logits,attn_key)
            ...
            vision_mask = (input_ids == self.config.image_token_id)[0]
            st_idx = torch.nonzero(vision_mask, as_tuple=True)[0]
        ...
        # TODO: optimize
        if select_pixel :
            select_mask = compression_mask & dyn_token_mask

            self.compression_rate = self.cal_compose_rate(compression_mask)
            self.dyn_token_rate = self.cal_compose_rate(dyn_token_mask)
            self.select_rate = self.cal_compose_rate(select_mask)

            vision_mask[st_idx] = ~select_mask
            vision_mask = ~vision_mask
            inputs_embeds = inputs_embeds[:,vision_mask]
            position_ids = position_ids[:,:,vision_mask]
            attention_mask = attention_mask[:,vision_mask]
            # Apply visual_pos_masks support
            visual_pos_masks = visual_pos_masks[:, vision_mask]
            # Apply same filtering to deepstack_visual_embeds
            if deepstack_visual_embeds is not None:
                filtered_deepstack_visual_embeds = []
                for layer_embeds in deepstack_visual_embeds:
                    # Each layer has corresponding embeds
                    filtered_deepstack_visual_embeds.append(layer_embeds[select_mask])
                deepstack_visual_embeds = filtered_deepstack_visual_embeds
