...
class VisionFlashAttention2(VisionAttention):
    ...
    def forward(
        ...
        if return_logits:
            with torch.no_grad():
                query_states = query_states.permute(1, 0, 2).split(batched_num_patches.tolist(), dim=1)
                key_states = key_states.permute(1, 0, 2).split(batched_num_patches.tolist(), dim=1)
                attn_logits = []
...
class HulumedVisionEncoderLayer(nn.Module):
    ...
    # Ignore copy
    def forward(self, hidden_states, cu_seqlens, rotary_pos_emb,batched_num_patches,return_logits: bool = False) -> torch.Tensor:
        attn, logits, attn_key = self.self_attn(
...
class HulumedVisionTransformerEncoder(nn.Module):
    ...
    def forward(self, hidden_states, grid_sizes, merge_sizes) -> torch.Tensor:
        ...
        for each in grid_sizes:
            tmp = each.repeat(each[0],1)
            tmp[:,0] = 1
            grid_sizes_.append(tmp)
        grid_sizes_ = torch.cat(grid_sizes_)
        batched_num_patches = grid_sizes_.prod(dim=1).long()
        ...
        for layer_num, blk in enumerate(self.layers):
            if self.gradient_checkpointing and self.training:
                ...
                if layer_num == len_layers-1:
                    hidden_states, logits, attn_key = blk(hidden_states, cu_seqlens=cu_seqlens, rotary_pos_emb=rotary_pos_emb, batched_num_patches=batched_num_patches,return_logits=True)
                else:
                    hidden_states, _, _ = blk(hidden_states, cu_seqlens=cu_seqlens, rotary_pos_emb=rotary_pos_emb, batched_num_patches=batched_num_patches,return_logits=False)
...
class HulumedVisionEncoderModel(PreTrainedModel):
    ...
    def forward(self, pixel_values, grid_sizes, merge_sizes=None) -> torch.Tensor:
        ...
        hidden_states, logits, attn_key = self.encoder(hidden_states, grid_sizes, merge_sizes)
        ...
        return torch.cat(outputs, dim=0),logits, attn_key
