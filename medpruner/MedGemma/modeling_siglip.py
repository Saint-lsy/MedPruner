        ...
        **kwargs,
        ...
        if return_logits:
            with torch.no_grad():
                ...
                attn_means = []
                attn_keys = []
                for querie, key in zip (queries,keys):
                    attn_logit = torch.matmul(querie, key.transpose(-1, -2))  # [batch, H, T, T]
                    attn_logit = attn_logit / (querie.shape[-1] ** 0.5)  
                    attn_logit = F.softmax(attn_logit, dim=-1)  # [batch, H, T, T]
                    
                    attn_mean = attn_logit.mean(dim=0)  
                    attn_mean = attn_mean.sum(dim=0) 
                    attn_mean = attn_mean.view(attn_mean.shape[0]//16, -1).mean(dim=-1)
                    
                    attn_key = key.view(key.shape[0], key.shape[1]//16, 16, key.shape[-1]).mean(dim=2)
        ...
        return attn_output, attn_weights, attn_means, attn_keys
...
class SiglipEncoderLayer(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        hidden_states, _ ,logits, attn_key = self.self_attn(
...
class SiglipEncoder(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        len_blocks = len(self.layers)
        for layer_num, blk in enumerate(self.layers):
            if layer_num == len_blocks - 1:
                hidden_states, logits, attn_key = blk(hidden_states, attention_mask, return_logits=True, **kwargs)
            else:
                hidden_states, _, _ = blk(hidden_states, attention_mask, return_logits=False, **kwargs)

        return BaseModelOutput(last_hidden_state=hidden_states), logits, attn_key
...
class SiglipVisionTransformer(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        encoder_outputs, attn_logits, attn_key = self.encoder(
            inputs_embeds=hidden_states,
            **kwargs,
        )
