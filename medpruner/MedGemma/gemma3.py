...
class Gemma3Model(Gemma3PreTrainedModel):
    ...
    def __init__(self, config: Gemma3Config, **kwargs):
        ...
        # Track if token filtering was applied to manage cache_position correctly
        self._filtered_seq_len = None
        self.post_init()
    ...
    def get_image_features(self, pixel_values: torch.Tensor) -> torch.Tensor:
        ...
        """
        vision_outputs, attn_mean, attn_key = self.vision_tower(pixel_values=pixel_values)
        image_features = self.multi_modal_projector(vision_outputs.last_hidden_state)

        return image_features, attn_mean, attn_key
    ...
    def dyn_token_compose(
        ...
        for mm_feature,attn_logit,attn_key in zip(inputs_embeds, attn_logits, attn_keys):
            num_tokens = attn_logit.size(0)
            device = attn_logit.device
            
            # Calculate selection ratios
            temperature = 2
            attn_probs = torch.nn.functional.softmax(attn_logit / temperature, dim=-1)
            contextual_num = max(int(0.05 * num_tokens), 1)
            tau = float(self.tau)
            topk_indices = get_mask_entropy(attn_probs, tau=tau)

            select_mask = torch.zeros_like(attn_logit, dtype=torch.bool)
            ...
            assign_one_hot = torch.zeros(
                ...
                dtype=attn_logit.dtype, device=device
            )
    ...
    def inter_slice_compse(
        ...
        merge_size: torch.FloatTensor = 4,
        ...
        # For Gemma3 image tower: image tokens per side are based on (patch_size * merge_size)
        tokens_h = images.shape[-2] // (int(patch_size) * int(merge_size))
        tokens_w = images.shape[-1] // (int(patch_size) * int(merge_size))
        num_tokens = tokens_h * tokens_w

        if t == 1:
            compression_masks.append(torch.ones((t * 256,), dtype=torch.bool, device=pixel_values.device))

        elif t >= 1:
            # Video token compression based on pixel differences
            images = pixel_values.view(t, (896 // merge_size // patch_size) * (896 // merge_size // patch_size), -1) # (t, P, D)
            ...
            # Pseudo image case
            compression_masks.append(torch.ones((0,), dtype=torch.bool, device=device))
    ...
    def forward(
        ...
        **lm_kwargs,
        ...
        if cache_position is None:
            # If we're in generation mode (cache is initialized and no new pixel_values),
            # use the tracked filtered sequence length instead of calculating from inputs_embeds
            is_generation_mode = (
                past_key_values is not None
                and past_key_values.is_initialized
                and pixel_values is None
                and self._filtered_seq_len is not None
            )

            if is_generation_mode:
                # During generation, use the filtered length to calculate correct positions
                past_seen_tokens = self._filtered_seq_len
                cache_position = torch.arange(
                    past_seen_tokens, past_seen_tokens + inputs_embeds.shape[1], device=inputs_embeds.device
                )
            else:
                # Normal calculation for prefill or first forward pass
                past_seen_tokens = past_key_values.get_seq_length() if past_key_values is not None else 0
                cache_position = torch.arange(
                    past_seen_tokens, past_seen_tokens + inputs_embeds.shape[1], device=inputs_embeds.device
                )
        else:
            # Override incorrect cache_position passed by generation framework
            # This happens when the framework calculated cache_position based on the original (pre-filtering) sequence length
            is_generation_with_wrong_cache = (
                past_key_values is not None
                and past_key_values.is_initialized
                and pixel_values is None
                and self._filtered_seq_len is not None
                and cache_position[0].item() > self._filtered_seq_len  # Wrong position detected
            )

            if is_generation_with_wrong_cache:
                old_position = cache_position[0].item()
                correct_position = self._filtered_seq_len
                cache_position = torch.arange(
                    correct_position, correct_position + inputs_embeds.shape[1], device=inputs_embeds.device
                )
                # CRITICAL: Also update position_ids for RoPE embeddings
                position_ids = cache_position.unsqueeze(0)

        select_pixel = False
        # Merge text and images
        if pixel_values is not None:
            image_features, attn_logits, attn_keys = self.get_image_features(pixel_values)
            image_features = image_features.to(inputs_embeds.device, inputs_embeds.dtype)

            compression_mask = torch.ones(len(pixel_values) * 256, dtype=torch.bool,device=input_ids.device)
            dyn_token_mask = torch.ones(len(pixel_values) * 256, dtype=torch.bool,device=input_ids.device)
            if self.slice_compose:
                compression_mask = self.inter_slice_compse(pixel_values)
            if self.token_compose:
                image_features, dyn_token_mask = self.dyn_token_compose(
                    image_features,attn_logits,attn_keys
                )
                image_features = image_features.to(inputs_embeds.device, inputs_embeds.dtype)
            ...
            # Renumber position_ids after filtering to maintain sequential order [0, 1, 2, ...]
            # This is critical for correct RoPE computation during autoregressive generation
            position_ids = torch.arange(inputs_embeds.shape[1], device=inputs_embeds.device).unsqueeze(0)

            # CRITICAL: Recalculate cache_position AFTER filtering to match the new sequence length
            # This ensures the KV cache and position embeddings are aligned with the filtered sequence
            past_seen_tokens = past_key_values.get_seq_length() if past_key_values is not None else 0
            cache_position = torch.arange(
                past_seen_tokens, past_seen_tokens + inputs_embeds.shape[1], device=inputs_embeds.device
            )

            # Track the filtered sequence length for use in generation steps
            self._filtered_seq_len = inputs_embeds.shape[1]
...
class Gemma3ForConditionalGeneration(Gemma3PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: Gemma3Config, **kwargs):
        super().__init__(config)
        self.model = Gemma3Model(config, **kwargs)
