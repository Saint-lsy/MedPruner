...
try:
    from .configuration_gemma3_encoder import SiglipVisionConfig
except ModuleNotFoundError:
    spec = importlib.util.spec_from_file_location(
        "configuration_hulumed_encoder",
        osp.join(osp.dirname(__file__), "configuration_gemma3_encoder.py"),
    )
    configuration_gemma3_encoder = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(configuration_gemma3_encoder)
    SiglipVisionConfig = getattr(
        configuration_gemma3_encoder,
        "SiglipVisionConfig",
    )
...
try:
    ...
    spec = importlib.util.spec_from_file_location(
        "modeling_siglip",
        osp.join(osp.dirname(__file__), "modeling_siglip.py"),
    )
    modeling_siglip = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(modeling_siglip)
    SiglipVisionModel = getattr(
        modeling_siglip,
        "SiglipVisionModel",
    )
