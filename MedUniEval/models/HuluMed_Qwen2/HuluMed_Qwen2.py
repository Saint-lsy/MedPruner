        ...
        for messages in tqdm(messages_list, desc="Generating Outputs"):
            ...
            if self.methed == "medpruner":
                compose_rates.append({
                    "compression_rate" : self.model.compression_rate.float().cpu().item(),
                    "dyn_token_rate" : self.model.dyn_token_rate.float().cpu().item(),
                    "select_rate" : self.model.select_rate.float().cpu().item()
                })
