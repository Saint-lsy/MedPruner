...
class Amos(BaseDataset):
    ...
    def run(self, samples, model):
        with torch.no_grad():
            ...
            for current_messages, current_samples in tqdm(messages_list, desc="Generating reports"):
                ...
                compose_rates = [None] * len(current_samples)

                if isinstance(outputs, tuple):
                    outputs, compose_rates = outputs
                try:
                    for sample, response, compose_rate in zip(current_samples, outputs, compose_rates):
                        del sample["messages"]
                        sample["response"] = response
                        if isinstance(compose_rate, dict):
                            sample["compose_rates"] = {
                                "compression_rate": compose_rate.get("compression_rate", 1.0),
                                "visionzip_rate": compose_rate.get("visionzip_rate", 1.0),
                                "select_rate": compose_rate.get("select_rate", 1.0),
                            }
                        else:
                            sample["compose_rates"] = {
    ...
    def load_data(self):
        ...
        all_lines = []
        with open(json_path + "/amos_val_mrg_sft.json","r") as f:
