  ...
  def run(self,samples,model,batch_size = 2000):
    ...
    with torch.no_grad():
        ...
        for current_messages,current_samples in tqdm(messages_list):
            outputs,compose_rates = model.generate_outputs(current_messages)
            try:
                for sample,response,compose_rate in zip(current_samples,outputs,compose_rates):
                    del sample["messages"]
                    sample["response"] = response
                    sample["compose_rates"] = compose_rate
  ...
  def eval(self):
    ...
    if num_chunks == 1:
        ...
        if chunk_idx == 0:
            ...
            for f in ["results.json", "metrics.json"]:
        ...
        total_results_path = [result for result in total_results_path 
                             if result.startswith("results_") and result.endswith(".json")]
        
        if len(total_results_path) == num_chunks:
            total_results = []
            for result in sorted(total_results_path):  
